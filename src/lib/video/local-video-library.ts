...
export class LocalVideoLibrary {
  ...
  async save(
    ...
    try {
      ...
      if (video) {
        const sourceHandle = await open(video.path, constants.O_RDONLY | NO_FOLLOW);
  ...
  async getRun(renderId: string): Promise<VideoManifest | null> {
    ...
    const handle = await open(manifestPath, constants.O_RDONLY | NO_FOLLOW);
    try {
  ...
  async readVideo(renderId: string): Promise<StoredVideo | null> {
    ...
    const handle = await open(videoPath, constants.O_RDONLY | NO_FOLLOW);
    const openedDetails = await handle.stat();
    if (!openedDetails.isFile()) {
      await handle.close();
      throw new Error("Unsafe video file");
    }
