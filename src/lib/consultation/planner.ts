...
export async function consult(context:ConsultationContext,ports:{ai:AiProvider;lookup:(org:string,query:EvidenceQuery)=>Promise<Evidence>;checkout?:{collect:(input:unknown,requestId:string)=>Promise<{ok:false;field:"items"|"variant"|"quantity"|"buyerName"|"phone"|"address"}|{ok:true;url:string}>};status?:{verify:(input:unknown)=>Promise<StatusVerifyResult>}},signal:AbortSignal):Promise<Outcome>{
 ...
 try{
  ...
  const families=specFamilies.filter(family=>family.match.test(context.text)).map(family=>family.names.map(name=>`spec:${name}`));
  if(evidence.products.some(p=>[...required].some(field=>!p.facts.some(f=>f.field===field))||families.some(family=>!family.some(field=>p.facts.some(f=>f.field===field)))||(plan.intent==="compare"&&p.missing.some(m=>m==="price_vnd"||m==="specifications_incomplete"||m.startsWith("conflicting_spec:")))))return gap("missing_evidence","specification",query);
  ...
  for(const source of evidence.knowledge){
   for(const chunk of source.chunks){for(const sentence of chunk.match(/[^.!?\n]+[.!?]?/gu)??[]){const excerpt=sentence.trim();
    ...
    const quoteSafe=mapped.length?mapped.every(p=>isSafeDescriptiveQuote(excerpt,p.facts.filter(f=>f.field.startsWith("spec:")).map(f=>({name:f.field.slice(5),value:String(f.value)})))):isSafeDescriptiveQuote(excerpt,[]);
    if(excerpt.length<20||excerpt.length>400||!quoteSafe||(/\d/u.test(excerpt)&&(!mapped.length||mapped.some(p=>p.missing.some(m=>m==="specifications_incomplete"||m.startsWith("conflicting_spec:"))))))continue;
  ...
  if(evidence.products.some(p=>[...required].some(field=>!chosen.some(f=>f.claim.productId===p.productId&&f.claim.variantId===p.variantId&&f.claim.field===field))||families.some(family=>!chosen.some(f=>f.claim.productId===p.productId&&f.claim.variantId===p.variantId&&family.includes(String(f.claim.field))))||(plan.intent==="compare"&&!chosen.some(f=>f.claim.productId===p.productId&&f.claim.variantId===p.variantId))))return gap("missing_evidence","specification",query);
