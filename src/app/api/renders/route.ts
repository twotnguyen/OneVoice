...
type Dependencies = Readonly<{
  ...
  pipeline: {
    create(command: { renderId: string; productId: string; scope: OrganizationScope }): Promise<RenderRun>;
  };
}>;
...
export function createRendersRoute(dependencies: Dependencies) {
  return {
    async POST(request: Request): Promise<Response> {
      ...
      try {
        const run = await dependencies.pipeline.create({ ...parsed.data, scope: dependencies.scope });
