...
import { campaignQuerySchema, manualCampaignSchema, unscheduleSlotSchema } from "@/lib/campaigns/management";
...
export async function POST(request: Request) {
 return withApiPermission(request, "manage_marketing", async actor => {
  ...
  const unschedule = unscheduleSlotSchema.safeParse(body);
  if (unschedule.success) {
   try { return Response.json(await createCampaignRepository(createSupabaseDataClient(), actor).unschedule(unschedule.data)); }
